  ...
  if (req.files && req.files.images) {
    ...
    for (const image of images) {
      const result = await cloudinary.uploader.upload(image.tempFilePath, {
        folder: "PERN-Ecommerce_Product_Images",
        width: 1000,
        crop: "scale",
      });
...
export const updateProduct = catchAsyncErrors(async (req, res, next) => {
  ...
  const result = await database.query(
    ...
    [name, description, price, category, stock, JSON.stringify(uploadedImages), productId]
  );
  res.status(200).json({
    success: true,
...
export const fetchAIFilteredProducts = catchAsyncErrors(
  async (req, res, next) => {
    ...
    //remove unnecessary words  example: I want a laptop
