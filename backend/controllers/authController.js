...
export const register = catchAsyncErrors(async (req, res, next) => {
  ...
  if (password.length < 4 || password.length > 16) {
    return next(
      new ErrorHandler("Password must be between 5 and 16 characters.", 400)
    );
  }
...
export const login = catchAsyncErrors(async (req, res, next) => {
  ...
  if (user.rows.length === 0) {
    return next(new ErrorHandler("User not registered with this email.", 400));
  }
...
export const forgotPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  try {
    await sendEmail({
      email: user.email,
      subject: "PERN-Ecommerce Password Recovery",
      message,
    });
    res.status(200).json({
      success: true,
...
export const resetPassword = catchAsyncErrors(async (req, res, next) => {
  ...
  if (
    ...
    req.body.confirmPassword?.length < 4 ||
